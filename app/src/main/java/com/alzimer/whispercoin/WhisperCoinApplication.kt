...
import com.alzimer.whispercoin.data.repository.AppLockRepository
...
@HiltAndroidApp
class WhisperCoinApplication : Application(), Configuration.Provider {
    ...
    private val applicationExceptionHandler = CoroutineExceptionHandler { _, throwable ->
        Log.e("WhisperCoinApplication", "Unhandled error in application scope", throwable)
        com.alzimer.whispercoin.utils.CrashHandler.triggerCrash(this, throwable)
    }
    ...
    override fun onCreate() {
        ...
        if (processName != null && processName.endsWith(":crash")) {
            Log.d("WhisperCoinApplication", "Started in :crash process. Skipping main initialization.")
            return
        }

        // Install crash handler first — must be before any other initialization
        com.alzimer.whispercoin.utils.CrashHandler.install(this)
        registerActivityLifecycleCallbacks(AppLockLifecycleObserver())
        applicationScope.launch {
            try {
                webhookSyncScheduler.applyScheduling()
            } catch (e: Exception) {
                Log.e("WhisperCoinApplication", "Error scheduling webhooks", e)
            }
        }
    }
