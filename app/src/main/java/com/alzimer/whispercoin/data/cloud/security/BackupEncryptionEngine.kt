package com.alzimer.whispercoin.data.cloud.security
...
class BackupEncryptionEngine @Inject constructor() {
        ...
        return try {
            FileInputStream(encryptedFile).use { fis ->
                ...
                if (headerRead != MAGIC_HEADER.length || headerString != MAGIC_HEADER) {
                    return Result.failure(IllegalArgumentException("Invalid encrypted backup file header. The file may not be an encrypted WhisperCoin backup."))
                }
    ...
    /**
     * Check if a file appears to be an encrypted WhisperCoin backup based on magic header.
     */
