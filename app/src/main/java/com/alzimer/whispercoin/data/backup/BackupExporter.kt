...
class BackupExporter @Inject constructor(
     ...
     */
    private suspend fun createBackup(config: BackupConfiguration): WhisperCoinBackup {
        ...
        val webhookProfiles = if (config.includeAppPreferences) {
            database.webhookProfileDao().getAllProfiles().first().map { profile ->
                WebhookProfileBackup(
                    ...
                    headers = com.alzimer.whispercoin.data.repository.WebhookHeaderEncoder.sanitizeForExport(
    ...
    private fun createBackupFile(): File {
        ...
        val fileName = "WhisperCoin_Backup_$timestamp.zip"
