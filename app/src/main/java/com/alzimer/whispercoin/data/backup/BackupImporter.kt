package com.alzimer.whispercoin.data.backup
...
import com.alzimer.whispercoin.data.database.entity.*
import com.alzimer.whispercoin.data.preferences.UserPreferencesRepository
import com.alzimer.whispercoin.data.repository.WebhookRepository
...
class BackupImporter @Inject constructor(
     ...
     */
    private suspend fun readBackupFile(uri: Uri): WhisperCoinBackup {
        return withContext(Dispatchers.IO) {
            // Try to read as ZIP first
            try {
                context.contentResolver.openInputStream(uri)?.use { inputStream ->
                    val zipInput = ZipInputStream(inputStream)
                    var backup: WhisperCoinBackup? = null
                    ...
                    if (entry != null) {
                        ...
                        while (entry != null) {
                            ...
                            if (name == "backup.json") {
                                ...
                                backup = gson.fromJson(content, WhisperCoinBackup::class.java)
            ...
            context.contentResolver.openInputStream(uri)?.use { inputStream ->
                ...
                gson.fromJson(content, WhisperCoinBackup::class.java)
     ...
     */
    private fun isCompatibleVersion(backup: WhisperCoinBackup): Boolean {
        // For now, accept all v1.x backups
        return backup.format.startsWith("WhisperCoin Backup v1")
     ...
     */
    private suspend fun replaceAllData(backup: WhisperCoinBackup): ImportResult {
     ...
     */
    private suspend fun mergeData(backup: WhisperCoinBackup): ImportResult {
        var importedTransactions = 0
        var importedCategories = 0
    ...
    private fun WebhookProfileBackup.toDraft(currency: String): com.alzimer.whispercoin.data.webhook.WebhookProfileDraft =
        com.alzimer.whispercoin.data.webhook.WebhookProfileDraft(
            id = id,
            name = name,
            url = url,
            enabled = enabled,
            dataTypes = dataTypes
                .mapNotNull {
                    runCatching {
                        com.alzimer.whispercoin.data.database.entity.WebhookDataType.valueOf(it)
                    }.getOrNull()
                }
                .toSet(),
